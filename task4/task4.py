...
def mixup_data(x,y,alpha=0.2):
    if alpha > 0:
        ...
        lam = 1
    batch_size = x.size(0)
    index = torch.randperm(batch_size).to(x.device)     #打乱索引
    mixed_x = lam * x + (1 - lam) * x[index, :]         #混合数据
...
def mixup_criterion(criterion, pred, y_a, y_b, lam):
    """Mixup损失计算"""
    # 直接使用CrossEntropyLoss，因为它是每个样本的平均损失
...
class WarmupScheduler :
    def __init__(self, optimizer, warmup_epochs,base_lr,num_epochs):
    ...
    def step(self, epoch):
        ...
        if epoch < self.warmup_epochs:
            warmup_bi_li = (epoch + 1) / self.warmup_epochs
            current_lr = self.base_lr_ * warmup_bi_li

            for param_group in self.optimizer.param_groups:      #获取当前学习率
               param_group['lr'] = current_lr                   #更改学习率（下面一样）
               print(f"Warmup阶段，Epoch {epoch + 1}: Warmup学习率 = {current_lr:.6f}")
            ...
            if (epoch + 1) % 5 == 0:
                ...
                for param_group in self.optimizer.param_groups:
                    ...
                    print(f"线性退火阶段,Epoch {epoch + 1}: Warmup学习率 = {current_lr:.6f}")

    def get_lr(self):
        return self.optimizer.param_groups[0]['lr']      #获取当前学习率

                            #4.构建训练函数和评估函数


def train_epoch(model, train_loader, criterion, optimizer, device, epoch, scheduler=None, mixup_alpha=0.2):
    """训练一个epoch"""
    model.train()
    ...
    for batch_idx, (inputs, targets) in enumerate(pbar):
        ...
        if mixup_alpha > 0:
            ...
            # 注意：在Mixup训练时，我们通常不计算batch内的准确率
            # 因为输入是混合的，标签也是混合的，准确率计算不准确
            correct = 0
            total = 0
        else:
            # 不使用Mixup
...
def visualize_learning_rate(learning_rates, warmup_epochs):
                    """可视化学习率变化"""
                    ...
                    # 绘制学习率曲线
                    plt.plot(epochs, learning_rates, 'b-', linewidth=2, label='学习率')

                    # 标记Warmup阶段
                    if warmup_epochs > 0:
                        plt.axvspan(0, warmup_epochs, alpha=0.2, color='orange', label='Warmup阶段')

                    plt.xlabel('Epoch')
                    plt.ylabel('学习率')
                    plt.title(f'学习率变化 (Warmup={warmup_epochs}个epoch)')
                    plt.legend()
                    ...
                    plt.tight_layout()
                    plt.savefig('learning_rate_history.png', dpi=150, bbox_inches='tight')
                    plt.show()

                    # 打印学习率统计信息
                    print(f"\n学习率统计:")
                    print(f"  初始学习率: {learning_rates[0]:.6f}")
                    print(f"  Warmup后学习率: {learning_rates[warmup_epochs - 1]:.6f}")
                    print(f"  最终学习率: {learning_rates[-1]:.6f}")
                    print(f"  最大学习率: {max(learning_rates):.6f}")
                    print(f"  最小学习率: {min(learning_rates):.6f}")
...
def main():
    ...
    # 添加Mixup超参数
    mixup_alpha = 0.2  # Mixup参数，设为0表示不使用Mixup

    for epoch in range(num_epochs):

        # 训练一个epoch
        train_loss, train_acc = train_epoch(
            model, train_loader, criterion, optimizer, device, epoch, scheduler,mixup_alpha=mixup_alpha
        )
        # 记录当前学习率
    ...
    print(f"  Mixup参数alpha: {mixup_alpha}")

    # 5. 可视化训练历史
    print("\n4. 可视化训练历史...")
    visualize_training_history(train_losses, train_accuracies, test_losses, test_accuracies)
    visualize_learning_rate(learning_rates, warmup_epochs)
